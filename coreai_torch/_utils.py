...
def _size_hint(size: Any) -> int:
    """An example size for ``size``, without constraining the ``ShapeEnv``.

    ``int(sym)`` would guard, specializing the symbol for every holder of the same
    ``ShapeEnv`` -- including the parent program being exported.
    """
    ...
    if hint is not None:
        return int(hint)
    # Unbacked symbol: no hint exists. Fall back to the low end of its range, which
    # is a legal example size and still adds no guard.
    rng = size.node.shape_env.var_to_range.get(size.node.expr)
    return max(int(rng.lower), 1) if rng is not None else 1
...
def _fake_inputs_from_node(node: fx.Node) -> tuple[torch.Tensor, ...]:
    ...
    Symbolic dims are resolved through their *hint* rather than ``int(sym)``.
    ``int()`` on a SymInt is a guard: it specializes the symbol in the
    ``ShapeEnv``, which is **shared with the parent program**, so the parent's
    dynamic dim silently becomes static -- one specialization per externalized call
    site. The symptom is remote from the cause: the parent converts with a static
    dim and only fails much later, when static shape specialisation rejects it
    ("coreai.enumerated_shapes key '...' for argument #0 modifies a static
    dimension"). Reading ``node.hint`` gives the same example size without
    ...
    for i, arg in enumerate(node.args):
        ...
        concrete_shape = [_size_hint(s) for s in val.shape]
